...
def patch_landmarker(mock_landmarker):
    """
    Parcha HandLandmarker.create_from_options
    en el módulo donde SE USA.
    """
    with patch(
...
def test_raw_stream_send_calls_detect_async(dummy_frame, patch_landmarker):
    callback = MagicMock()
...
def test_raw_stream_internal_callback_passes_raw_result(patch_landmarker):
    callback = MagicMock()
...
def test_raw_stream_close_releases_resources(patch_landmarker):
    callback = MagicMock()
...
def test_wrapped_stream_send_calls_detect_async(dummy_frame, patch_landmarker):
    callback = MagicMock()
...
def test_wrapped_stream_internal_callback_wraps_result(patch_landmarker):
    callback = MagicMock()
    raw_result = MagicMock()
    mp_image = MagicMock()
    timestamp = 321
...
def test_wrapped_stream_close_releases_resources(patch_landmarker):
    callback = MagicMock()
